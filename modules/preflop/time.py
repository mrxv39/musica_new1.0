...
def main():
    try:
        ...
        fingerprint = hashlib.sha1((abs_image_path + "|time|" + str(int(t.time())//2)).encode()).hexdigest()
        out = {"time_ok": time_ok, "score": score, "fingerprint": fingerprint}
        ...
        fingerprint = hashlib.sha1((abs_image_path + "|time|" + str(int(t.time())//2)).encode()).hexdigest()
        out = {"time_ok": False, "score": 0.0, "fingerprint": fingerprint, "error": str(e)}
        print(json.dumps(out))
