...
def test_run_worker_mesa_once_skip_si_time_gate_false(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda area, tmp_dir, ts: {"time_ok": False, "score": 0.1, "reason": "clock_not_visible"},
    )

    called = {"capture": 0, "insert": 0}

    monkeypatch.setattr(wmod, "capture_to_tmp", lambda *a, **k: called.__setitem__("capture", called["capture"] + 1))
...
def test_run_worker_mesa_once_skip_duplicado_por_fingerprint(monkeypatch, tmp_path):
    ...
    img_path = os.path.join(dirs.tmp_dir, "cap.bmp")
    _write_file(img_path, b"dup")
...
def test_run_worker_mesa_once_si_falla_capture_ocr_loguea_y_sale(monkeypatch, tmp_path):
    ...
    txt = fp.getvalue()
    assert "CAPTURE/OCR ERROR:" in txt
    assert "ocr_boom" in txt
    assert not os.path.exists(img_path)
