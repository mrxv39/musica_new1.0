...
class TestOCROrchestrator(unittest.TestCase):
    @patch('modules.ocr.stackefectivo.read_stackefectivo')
    ...
    def test_ocr_orchestration(self, mock_villano, mock_names, mock_stacks, mock_bets, mock_stackefectivo):
        # Set up deterministic return values
        mock_stackefectivo.return_value = {'ok': True, 'val': 1, 'errors': []}
        ...
        self.assertEqual(out['stackefectivo']['ok'], True)
    ...
    @patch('modules.ocr.stackefectivo.read_stackefectivo', side_effect=Exception('fail_stack'))
    ...
    def test_ocr_all_failures(self, mock_villano, mock_names, mock_stacks, mock_bets, mock_stackefectivo):
        out = ocr.run_ocr('img.png')
        self.assertFalse(out['ok'])
        self.assertIn('stackefectivo:fail_stack', out['errors'])
        self.assertIn('bets:fail_bets', out['errors'])
